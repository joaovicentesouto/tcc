...
\label{sec.multiple-processor-systems}
	...
	\label{sec.multiprocessors}
		...
		\todo[inline]{Eu acho que as figuras são adaptações das figuras do livro do tannenbaum ou de outra fonte. Portanto, não podes dizer ``developed by the author'' no caption. O mais correto é dizer ``Adapted from [REF]''. Vale para essa e para todas as outras figuras do texto.}
		
		\todo[inline]{Em textos, normalmente as figuras são colocadas no topo ([t]) ou em baixo ([b]). Evite de usar [h] ou [H]. Reposicione as figuras somente após o texto estar finalizado.}
		...
		\label{sec.multiprocessor-hw}
			...
			Currently, two categories of multiprocessors attract attention, the \cmp and \todo{MPSoCs?}{\soc}.
			...
			\todo[inline]{Faltou adicionar um parágrafo para dizer que essas redes de
			interconexão nos MPSoCs são as NoCs. Adicionar duas ou três frases para
			falar de NoCs.}
	...
	\label{sec.multicomputers}
		...
		making things simpler~\cite{tanenbaum:4ed}.
			...
			\label{sec.multicomputers-user-sw}
				...
				According to \todo{citação aqui}{Tanenbaum}, there are four ways to implement a send primitive:
				\begin{itemize}
...
\label{sec.nanvix}
	...
	\todo[inline]{Adicionar alguns parágrafos para falar do Nanvix OS de modo geral (visão geral, microkernel, multikernel, etc..).}
	...
	\subsection{Hardware Abstract Layer (HAL)}
		...
		\end{figure}
	        	...
	        	\todo[inline]{Colocar a versão em formato vetorial (PDF) da figura. REF COMPASS 2019 está faltando.}
		...
		\todo[inline]{Assim como no HAL, colocar uma descrição geral dos serviços de comunicação que o
		Nanvix vai oferecer. A descrição de como isso será implementado no MPPA vai estar no capítulo de
		proposta.}