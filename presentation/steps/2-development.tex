\section{Development}
	...
	\subsection{Low-Level Communication}
		...
		\begin{frame}[fragile]{Sync Abstration}
			...
			\only<1>{
				\begin{itemize}
					...
					\item Similar to \textbf{POSIX Signals}
					\item Two modes
					\begin{itemize}
		...
		\begin{frame}[fragile]{Sync Abstration Implementation}
			\begin{itemize}
				\item Uses only \textbf{CNoC} Resources
		...
		\begin{frame}[fragile]{Mailbox Abstration Implementation}
			\begin{itemize}
				\item Merges the use of \textbf{DNoC and CNoC}
	...
	\subsection{User-Level Communication}
		...
		\begin{frame}[fragile]{Impacts of the Master-Slave Model}
			\begin{itemize}
				\item \textbf{Only Master changes the internal structures} of the OS
				\begin{itemize}
					\item Slaves need to read lock addresses to wait for asynchronous calls
				\end{itemize}
			\end{itemize}

			\begin{itemize}
				\item \textbf{Naming Convention} (Prefixes)
				\begin{itemize}
					\item Master calls: \texttt{\textbf{kernel}\_\textit{abstraction}\_\textit{operation}}
					\item Slave calls: \texttt{\textbf{k}\textit{abstraction}\_\textit{operation}}
				\end{itemize}
			\end{itemize}
