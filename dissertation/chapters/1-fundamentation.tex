...
\label{ch.fundamentation}
	...
	\todo[inline]{
		I simply moved the paragraphs as Odorico suggested.
		So I need to rethink these first 3 paragraphs.
	}
	...
	\label{sec.multiprocessors}
		...
		\todo[inline]{
			The explanation of concurrency issues in this paragraph is confusing.
			LOOKS IN THE PDF (Odorico's revision).
		}
		...
		\label{sec.multiprocessor-hw}
			...
			\todo{
				It needs a proper citation about the paragraph's theme here.
			}
		...
		\label{sec.multiprocessor-os}
			...
			\begin{figure}[!tb]
				...
				\todo[inline]{Master talvez só rode o OS, verificar.}%
...
\label{sec.mppa}
	...
	The hardware does not support cache coherence to improve energy consumption.
		\todo{Unnecessary On the other hand}
...
\label{sec.nanvix}
	...
	of processors~\cite{wentzlaff_factored_2009, baumann_multikernel:_2009, Wisniewski2014}.
	...
	\textit{Nanvix \microkernel}, and textit{Nanvix \multikernel}.
	...
	\label{sec.hal}
		...
		\begin{figure}[!tb]
			...
			\includegraphics[width=.6\textwidth]{nanvix-hal-overview.pdf}%
			\fonte{VER COM O PEDRO COMO REFERENCIAR (REPOSITÓRIO NANVIX?).}%
		\end{figure}
	...
	\label{sec.microkernel}
		...
		\begin{figure}[!tb]
			...
			\includegraphics[width=.6\textwidth]{nanvix-microkernel-overview.pdf}%
			\fonte{VER COM O PEDRO COMO REFERENCIAR (REPOSITÓRIO NANVIX?).}%
		\end{figure}
		...
		\begin{description}
			...
			\item[IPC Facility]
				...
				manipulation and protection of the inter-cluster communication primitives.
			...
			\item[Memory System]
				...
				It is based in a two-level paging scheme, supports pages of heterogeneous sizes and uses a
				capabilities system.
			...
			\item[Kernel Call Interface]
				...
				This interface is what really separates user execution from kernel execution.
	...
	\label{sec.multikernel}
		...
		Nanvix \multikernel is inspired by research that explores the parallel and distributed nature
		of multicore and manycore processors~\cite{wentzlaff_factored_2009, baumann_multikernel:_2009, Wisniewski2014}.
		By treating the processor as a network of independent cores, it is possible to cover concepts
		of modern distributed systems by implementing a client-server workload.
		So, multikernel design splits \os services into server processes that run in
		isolation from user processes.
		Through the message-passing approach, user processes request the kernel services.

		At this level of abstraction, it is possible to exploit distributed system concepts as:
		...
		\autoref{fig:multikernel-overview} shows a possible configuration of a manycore using the Nanvix \multikernel.
		Clusters located at the corners of the processor run multikernel services to serve a nearby subset of clusters better.
		...
		Now, looking carefully at a cluster, Nanvix \microkernel always reserves a single core for kernel execution,
		making the other cores available for application execution.

		\begin{figure}[!tb]
			\centering%
			\caption{Possible configurations on Nanvix \multikernel.}%
			\label{fig:multikernel-overview}%
			\includegraphics[width=.6\textwidth]{nanvix-multikernel-overview.pdf}%
			\fonte{VER COM O PEDRO COMO REFERENCIAR (REPOSITÓRIO NANVIX?).}%
		\end{figure}

		To enable Nanvix \os compliance with \posix, we can create services that implement standardized interfaces.
		From this perspective, user applications are easily ported to run on Nanvix \multikernel, where client-side
		interfaces abstract the communication with servers distributed on the processor.
		\autoref{fig:multikernel-shm} exemplifies the existing layers to provide the \shm service.
		When opening an \shm, the client does not have to worry about where data will be allocated or whether
		other cores in other clusters are operating over the same memory region.
		Consequently, we provide greater programmability and software portability for manycores processors.
