...
\label{sec.mppa}
	...
	\todo[inline]{Acho que seria interessante coloca rum paragrafo aqui
		para fazer um gancho onde no embasamento teorico o MPPA se
			encontra: ele é um processoador com multiplos núcleos, mas
			possui uma arquitetura de memória distribuída onde
			comunicação é feita por passagem de mensagem. Essa idéia já
			é reforçada no texto a seguir, mas o ponto seria fazer um
			gancho com a seção anterior, sem citar features técnicas}
...
\label{sec.nanvix}
	...
	\begin{figure}[!tb]
		...
		\fonte{VER COM O PEDRO COMO REFERENCIAR.}%
	\end{figure}
	...
	\label{sec.multikernel}
		...
		\todo[inline]{Em geral, as figuras do capítulo foram adaptadas
			de outros trabalhos. Entao você pode dizer adaptado de e
				coloca a referência ;-)}
