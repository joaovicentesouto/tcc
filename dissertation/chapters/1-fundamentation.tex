...
\label{sec.nanvix}
	...
	Durante as décadas de 70 e 80, começaram a surgiu várias iniciativas de sistemas operacionais.
	Para evitar que os sistemas operacionais fossem incompatíveis entre si, a padronização POSIX foi criada.
	A POSIX define interfaces e características esperadas de um sistema operacional.
	Por exemplo, o comportamento da criação e controle de processos e threads.
	Muitos desses sistemas deixaram de existir e novos surgiram, porém, a POSIX conseguiu se consolidar e se modernizar com o tempo até os dias de hoje. 
	
	A grande maioria dos sistemas operacionais atuais é projetado para trabalhar com uma quantidade pequenas de núcleos.
	Conforme a quantidade de núcleos aumenta, certas partes do sistema operacional requerem que sejam reprojetadas.
	Chegará a hora que esse retrabalho será inviável.
	Essa constatação levou pesquisadores a estudar alternativas de design para essa nova era de processadores~\cite{FOS, MOS}.
		\todo[inline]{Repensar a frase a baixo, "Entretanto, ..."}
	Entretanto, o foco desses sistemas operacionais são direcionados ao melhor desempenho, deixando que aspectos do hardware e decisões de design interfiram em suas interfaces ou em seu comportamento.
	...
	solution is to rethink \os design from scratch~\cite{penna:compas19, penna2019} SEM PERDER A COMPATIBILIDADE COM OUTROS SISTEMAS.

	\begin{figure}[!tb]
		\centering%
		\caption{Conceptual Goals of the Nanvix \os.}%
		\label{fig:nanvix-goal}%
		\includegraphics[width=.8\textwidth]{nanvix-goal.pdf}%
	...
	\label{sec.hal}
		...
		\todo[inline]{
			This section should focus on showing what is already available.
			Leave the contribution of the work to the development section.
		}
		...
		and \hero~\cite{Kurth2017} platforms.
			\todo{Insert others platforms? x86, OpenRISC, etc.}
		...
		\begin{figure}[!tb]
			...
			\includegraphics[width=.8\textwidth]{nanvix-hal-detailed.pdf}%
			\fonte{VER COM O PEDRO COMO REFERENCIAR (REPOSITÓRIO NANVIX?).}%
		\end{figure}
	...
	\label{sec.microkernel}
		...
		\begin{figure}[!tb]
			...
			\includegraphics[width=.85\textwidth]{nanvix-microkernel-overview.pdf}%
			\fonte{VER COM O PEDRO COMO REFERENCIAR (REPOSITÓRIO NANVIX?).}%
		\end{figure}
	...
	\label{sec.multikernel}
		...
		\begin{figure}[!tb]
			...
			\includegraphics[width=.8\textwidth]{nanvix-multikernel-overview.pdf}%
			\fonte{VER COM O PEDRO COMO REFERENCIAR (REPOSITÓRIO NANVIX?).}%
		\end{figure}
		...
		\begin{figure}[!tb]
			...
			\subcaptionminipage[fig:uma_a]%
				{.4\linewidth}%
				{Server cluster.}%
				{\includegraphics[width=\linewidth]{nanvix-multikernel-service.pdf}}%
			\hspace{1.5cm}%
			\subcaptionminipage[fig:uma_b]%
				{.4\linewidth}%
				{Application Cluster.}%
				{\includegraphics[width=\linewidth]{nanvix-multikernel-application.pdf}}%
