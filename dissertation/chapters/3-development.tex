...
% Quais recursos são necessário para cada tipo de operação, envio e recebimento?
				O recebimento das notificações requer a reserva 1 slot rx da CNoC.
				...
				Caso todos os bits se tornem 1's, a DMA emite uma notificação e zera todos os bits do registrador.

% Como as operações funcionam?
				Um vetor de estruturas internas realiza o controle das operações.
				Cada possíção do vetor é reservado a um slot físico e guarda flags de controle, uma mascará de 64-bits e um spinlock.
				Ao realizar a abertura, espera ou unlink de um sync, a HAL verifica as flags  alocado, ocupado, se é apenas leitura ou apenas escrita.
				A mascará é construída com os IDs dos emissores e é reutilizada para reconfigurar a DMA quando disparadá uma interrupção, por exemplo, a mascará para os emissores [0,2,4] é 0xFF...FFEA.
				...
				A falta de coerência de cache não afeta os spinlock porque são utilizadas instruições especiais para garantir o comportamento padrão de um lock.

% Como foi resolvido o problema de multiplos nós nos IOs?
				A implementação nos Cluster I/O requeriu 

\begin{listing}[!tb]
\caption{Nanvix HAL: Sync Interface for Receiver Node.}
\label{code:hal-sync-receiver}
\begin{minted}{c}
#define SYNC_CREATE_MAX (N_OF_DMA_AVAIABLES * N_OF_NOC_NODES)
