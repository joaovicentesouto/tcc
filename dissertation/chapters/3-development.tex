				...
				\begin{algorithmic}[1]
					...
					\For {$i \in [1, M_{Interfaces}]$}
						\For {$j \in [1, N_{Resources}]$}
							\If {$flags[i][j] == Interrupt\_Triggered$}
								\State {$\Call{clean\_flags}{i, j}$}   \Comment{Releases to receive new interrupts.}
								\State {$handlers[i][j](i, j)$}         \Comment{Runs the associated handler.}
							\EndIf
						\EndFor
					\EndFor
					\EndProcedure
...
\end{listing}
	...
	\label{sec.user-level-comm}
		...
		\subsection{Protection and Management}
			...
			The master phase performs more robust checks than those performed
		...
		\subsection{Validation and Correctness Tests}
			...
			Extra effort has been made to ensure that deployment across the
