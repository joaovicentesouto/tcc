...
\begin{resumo}
    ...
    As decisões de projeto no desenvolvimento de qualquer aplicação influencia
...
\label{cap:objetivos}
    ...
    Utilizando os conceitos apresentados no capítulo anterior, o presente
    trabalho tem como objetivo propor um ambiente de execução alternativo que
    equilibre flexibilidade, funcionalidade, desempenho e eficiência energética
    para o processador MPPA-256.
    Desta forma, ao utilizar camadas mais próximas do hardware e exportar e
    ...
    desenvolver em explorar otimizações para o domínio de aplicações de alto
    desempenho e, ao mesmo tempo, não aumentando a complexidade do projeto por
    não trabalhar diretamente com um hardware específico.
        ...
        \begin{figure}[t]
        	\begin{center}
            	\caption{Arquitetura \textit{Exokernel}.}
                   \label{figexokernel}
        		\begin{tabular}{ccc}
            	    \includegraphics[width=0.5\textwidth]{figs/exokernel.png} \\
        		\end{tabular}
                \vspace{1ex} \\
